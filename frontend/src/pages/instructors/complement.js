...
export const Complement = () => {
  ...
  const getComplements = async () => {
    try {
      const request = await axios.get(`https://ofcourse.website/api/v2/complements/user/${localStorage.getItem('_id')}/pagination/${pagination}`);
  ...
  const createOwn = async (event) => {
    ...
    try {
      ...
      reader.onload = function (e) {
        ...
        const request = axios.post(`https://ofcourse.website/api/v2/complements/`,{
          file: base64,
          fileName: str,
  ...
  const handleSubmit = async (event) => {
    await event.preventDefault()
    try{
        await axios.post(`https://ofcourse.website/api/v2/awards/`,{complementId:selected._id,_id:id,message:text,creatorId:localStorage.getItem('_id'),title:title})
        nav(`/courses`)
    }catch(err){
        console.log(err)
