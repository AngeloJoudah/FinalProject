...
export const Profile = () => {
    ...
    const getUserData = async() =>{
        try{
            const request = await axios.get(`https://ofcourse.website/api/v2/users/id/${personId}`)
    ...
    const checkIsStudent = async() =>{
        try{
            await axios.get(`https://ofcourse.website/api/v2/users/id/${localStorage.getItem('_id')}/student/${personId}`)
            setIsStudent(true)
        }catch(err){
            console.log(err)
        }
